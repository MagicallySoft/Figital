...
export default function Address() {
  ...
  // Toggle edit mode for a specific address by id
  const handleEditToggle = (id) => {
    setAddresses((prev) =>
      prev.map((address) =>
        address.id === id
    ...
    console.log("Toggled edit mode for address id:", id);
  };
    ...
    <div className="my-account-content">
      <div className="account-address">
        <div className="widget-inner-address ">
          ...
          <p>
            Review, add, or update your shipping addresses below. Keeping your
            address details current helps ensure smooth delivery.
          </p>
          {/* Add New Address form would go here */}
          <div className="list-account-address pt-3">
            {addresses.map((address) => (
              <div className="account-address-item" key={address.id}>
                ...
                <div className="address-actions d-flex gap-10 justify-content-start pt-3">
                  <button
                    className="tf-btn radius-4 btn-fill btn-edit-address"
                    onClick={() => handleEditToggle(address.id)}
                  >
                    <span className="text">
                      {address.isEditing ? "Close Edit" : "Edit"}
                    </span>
                  </button>
                  <button
                    className="tf-btn radius-4 btn-outline btn-delete-address"
                    onClick={() => handleDelete(address.id)}
                  >
                    <span className="text">Delete</span>
                  </button>
