...
export default function Products15({ parentClass = "flat-spacing" }) {
  ...
  const loadingProducts = useSelector(selectLoading);
  ...
  // Update loadedItems: append new items when sorted or page changes
  useEffect(() => {
    // For first page, reset loadedItems; for subsequent pages, append new items
    if (pagination.current_page === 1) {
      setLoadedItems(sorted);
    } else {
      setLoadedItems((prev) => {
        const newItems = sorted.filter((item) => !prev.includes(item));
        return [...prev, ...newItems];
      });
    }
  }, [sorted, pagination.current_page]);

  // Updated handleLoad: dispatch fetchProducts for the next page
  const handleLoad = () => {
    if (pagination.current_page < pagination.last_page) {
      dispatchs(fetchProducts(pagination.current_page + 1));
    }
  };

  const elementRef = useRef(null);

  // Intersection Observer to auto-load next page when button comes into view
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
  ...
  }, [loading, pagination.current_page]);
