...
export default function FilterMeta({ allProps, productLength }) {
  return (
    <div className="meta-filter-shop">
      ...
      <div id="applied-filters">
        {allProps.category !== "All" && (
          <span className="filter-tag" onClick={() => allProps.setCategory("All")}>
            {allProps.category}
            <span className="remove-tag icon-close" />
          </span>
        )}
        ...
        {allProps.color !== "All" && (
          <span className="filter-tag color-tag" onClick={() => allProps.setColor("All")}>
            <span className={`color bg-red ${allProps.color.className}`} />
            {allProps.color.name}
            <span className="remove-tag icon-close" />
          </span>
        ...
        {allProps.brands.length > 0 && (
          <>
            {allProps.brands.map((brand, i) => (
              <span key={i} className="filter-tag" onClick={() => allProps.removeBrand(brand)}>
                {brand}
                <span className="remove-tag icon-close" />
              </span>
            ))}
      ...
      {(
        ...
        allProps.color !== "All" ||
        allProps.brands.length > 0) && (
          <button id="remove-all" className="remove-all-filters text-btn-uppercase" onClick={allProps.clearFilter}>
            REMOVE ALL <i className="icon icon-close" />
          </button>
        )}
    </div>
