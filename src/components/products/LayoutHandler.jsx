...
export default function LayoutHandler({ activeLayout, setActiveLayout, hasSidebar = false }) {
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 1200 && window.innerWidth > 767) {
  ...
  return (
    <>
      <li
        className={`tf-view-layout-switch sw-layout-list list-layout ${activeLayout === 1 ? "active" : ""}`}
        onClick={() => setActiveLayout(1)}
        data-value-layout="list"
      >
        <div className="item">
      ...
      {!hasSidebar && (
        <li
          className={`tf-view-layout-switch sw-layout-5 ${activeLayout === 5 ? "active" : ""}`}
          onClick={() => setActiveLayout(5)}
          data-value-layout="tf-col-5"
        >
          <div className="item">
